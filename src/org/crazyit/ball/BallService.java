...
public class BallService {
    ...
    public BallService(BallFrame frame, int width, int height)
        ...
        // 创建一个Stick(档板)实例
        stick = new Stick(width, height, "D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/stick.jpg");
        // 创建一个弹球的实例
        ball = new Ball(width, height, stick.getImage().getHeight(null),
                "D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/ball.gif");
        // 游戏结束图片
        gameOver = new BallComponent("D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/over.gif");
        // 赢图片
        won = new BallComponent("D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/win.gif");
        // 砖块图片数组
        bricks = createBrickArr("D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/brick.gif", 11, 6);
    }
/**
 *
 * */
    ...
    public void setBallPos() {
        ...
        // 正数的数度
        //HEI:二次修改后,没有使用此变量,因为每帧位移很小,足够判断小球与边框距离,不需要添加位移补偿
        ...
        */
        ...
        if (ball.isStarted()) {
            ...
            // 如果小球碰到右边界
            if (ball.getX() >= width - ball.getImage().getWidth(null)) {

                // 重新设置x坐标
                ball.setX(width - ball.getImage().getWidth(null));
                // 把x方向的速度设为反方向
                ball.setSpeedX(-ball.getSpeedX());
            }
            ...
            if (ball.getY() <= 0) {
                ...
                // 重新设置y坐标
                ball.setY(ball.getImage().getHeight(null));
                // 把y方向的速度设为反方向
                ball.setSpeedY(-ball.getSpeedY());
            }

            // 如果小球碰到下边界结束游戏
            if (ball.getY() + ball.getImgHeight() > height) {
                ball.setStop(true);
            ...
            if (isHitStick(ball)) {
                ...
                // 重新设置y坐标
                //ball.setY(height - ball.getImage().getHeight(null) * 2);  //原作者
                ball.setY(height - ball.getImage().getHeight(null) - stick.getImgHeight()); //HEI
                // 把y方向的速度设为反方向
                ball.setSpeedY(-ball.getSpeedY());
                //HEI:记分
                score.setScore(10);
 ...
 /*           // 如果小球碰到下边界
            if (ball.getY() >= height) {
//                System.out.println("球的X位置" + ball.getX() + "球的Y位置" + ball.getY());//测试
//                System.out.println(ball.getHeight() + "    " + (height - stick.getPreHight()));//测试
                // 重新设置y坐标
                ball.setY(height - ball.getImage().getHeight(null) * 2);  //原作者
                 把y方向的速度设为反方向
                ball.setSpeedY(-ball.getSpeedY());
                System.out.println("小球碰到挡板了");
            ...
            /*
             * 更新ball的x和y轴位置,带上speed参数让小球以speed移动
             * 注释掉的内容为:增加随机变速效果,使得每次小球飞行角度小幅改变
             */
    ...
    /*    //HEI:brick对角线的长度
        double brickDiagonal =*//*砖块对角线的长度*//* Math.sqrt(*//*砖块宽的平方*//*Math.pow(brick.getImgWidth(), 2) + *//*砖块高的平方*//*Math.pow(brick.getImgHeight(), 2));
        //HEI:ball对角线长度
        double ballDiagonal =*//*球对角线的长度*//* Math.sqrt(*//*球宽的平方*//*Math.pow(ball.getImgWidth(), 2) + *//*球高的平方*//*Math.pow(ball.getImgHeight(), 2));*/
     ...
     *
     * @param "image" BallComponent 图像
     * @return boolean
     */
    public boolean isHitStick(BallComponent bc) {
        // 获取图片对象
        // Image tempImage = bc.getImage(); //取消获取,将返回图像高度和宽度的方法写入ballComponent对象中
        // 如果与档板有碰撞
        //原作者
        /*if (bc.getX() + tempImage.getWidth(null) > stick.getX()
            ...
            System.out.println("球高度" + (bc.getY() + tempImage.getHeight(null)) + "板子高度" + stick.getY());
            return true;
        }*/

        //HEI:修改后
        if (bc.getX() >= stick.getX() && (bc.getX() + bc.getImgWidth()) <= (stick.getX() + stick.getPreWidth())) {
            if ((bc.getY() + bc.getImgHeight()) > stick.getY()) {
//                System.out.println("小球碰到挡板了");//测试
                return true;
            }
        }
        return false;
