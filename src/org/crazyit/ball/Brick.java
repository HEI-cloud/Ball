...
public class Brick extends BallComponent {
	...
	public Brick(String path, int type, int x, int y) throws IOException {
		...
		if (type == Brick.MAGIC_LONG_TYPE) {
			this.magic = new LongMagic("D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/long.gif", x, y);
		} else if (type == Brick.MAGIC_SHORT_TYPE) {
			this.magic = new ShortMagic("D:/IT/IntelliJ IDEA 2020.2/Workspace/ball/img/short.gif", x, y);
		}
		if (this.magic != null) {
			this.magic.setX(x);
			this.magic.setY(y);
